...
<div id="app" class="centered_input_wide">
    ...
    <p><form action="kakutei.php" method="get"><input type="submit" value="レジに進む(〇個の商品)[税込み]" id="button"><input type="hidden" name="total_price" v-model="price"><span v-for="(post_item,s) in items" :post_key="s"><input type="hidden" name="purchase_pd[]" :value="post_item.product_id"><input type="hidden" name="purchase_amount[]" v-model="amounts[s]"></span></form></p>
        ...
        <div v-for="(item,i) in items" :key="i" class="purchase_product">
            ...
            <p>￥{{item.price}}</p>
            <p>{{item.cart_date}}</p>
            <form><button @click="delete_btn(item.product_id,i)" class="button_de" type="submit">削除</button></form>
            <div class="stepper-app">
                <button class="decrease_btn" @click="decrease(i)">-</button><div class="count"><input type="number" v-model="amounts[i]"></div><button @click="increase(i)" class="increase_btn">+</button>
            </div>
        </div>
</div>
